...
function HomeToaster() {
  ...
  useEffect(() => {
    if (isToastType(toastType)) {
      ...
      if (shownToastsRef.current.has(toastType)) {
        removeUrlParam();
        return;
      }

      // Mark this toast as shown
      shownToastsRef.current.add(toastType);
      ...
      // Clean up the ref after a delay to allow for future toasts of the same type
      setTimeout(() => {
        shownToastsRef.current.delete(toastType);
      }, 1000);
    }
  }, [toastType]);
